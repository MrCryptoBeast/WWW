dnl Copyright (c) 2013-2016 The worldwideweb Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set worldwideweb_enable_qt to false. Else, exit.
AC_DEFUN([worldwideweb_QT_FAIL],[
  if test "x$worldwideweb_qt_want_version" = xauto && test "x$worldwideweb_qt_force" != xyes; then
    if test "x$worldwideweb_enable_qt" != xno; then
      AC_MSG_WARN([$1; worldwideweb-qt frontend will not be built])
    fi
    worldwideweb_enable_qt=no
    worldwideweb_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([worldwideweb_QT_CHECK],[
  if test "x$worldwideweb_enable_qt" != xno && test "x$worldwideweb_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl worldwideweb_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([worldwideweb_QT_PATH_PROGS],[
  worldwideweb_QT_CHECK([
    if test "x$3" != x; then
      AC_PATH_PROGS($1,$2,,$3)
    else
      AC_PATH_PROGS($1,$2)
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      worldwideweb_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other worldwideweb_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([worldwideweb_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build worldwideweb-qt GUI (default=auto)])],
    [
     worldwideweb_qt_want_version=$withval
     if test "x$worldwideweb_qt_want_version" = xyes; then
       worldwideweb_qt_force=yes
       worldwideweb_qt_want_version=auto
     fi
    ],
    [worldwideweb_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); worldwideweb_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   worldwideweb_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _worldwideweb_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: worldwideweb_enable_qt, worldwideweb_enable_qt_dbus, worldwideweb_enable_qt_test
AC_DEFUN([worldwideweb_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  worldwideweb_QT_CHECK([_worldwideweb_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _worldwideweb_QT_CHECK_STATIC_PLUGIN does
  ...
  _worldwideweb_QT_IS_STATIC
  if test "x$worldwideweb_cv_static_qt" = xyes; then
    _worldwideweb_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      ...
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _worldwideweb_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  worldwideweb_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  worldwideweb_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  worldwideweb_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  worldwideweb_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  worldwideweb_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  worldwideweb_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  worldwideweb_QT_CHECK([
    worldwideweb_enable_qt=yes
    worldwideweb_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      worldwideweb_enable_qt_test=no
    fi
    worldwideweb_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      worldwideweb_enable_qt_dbus=yes
    fi
  ...
  if test x$worldwideweb_enable_qt = xyes; then
    AC_MSG_RESULT([$worldwideweb_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$worldwideweb_enable_qt])
  fi
...
dnl
dnl _worldwideweb_QT_IS_STATIC
dnl ---------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
dnl Output: worldwideweb_cv_static_qt=yes|no
AC_DEFUN([_worldwideweb_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, worldwideweb_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [worldwideweb_cv_static_qt=yes],
      [worldwideweb_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _worldwideweb_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_worldwideweb_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); worldwideweb_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _worldwideweb_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_worldwideweb_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _worldwideweb_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_worldwideweb_QT_FIND_LIBS],[
  worldwideweb_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [worldwideweb_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  worldwideweb_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [worldwideweb_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  worldwideweb_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [worldwideweb_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  worldwideweb_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [worldwideweb_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
