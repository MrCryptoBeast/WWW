# Bootstrappable worldwideweb Core Builds

This directory contains the files necessary to perform bootstrappable worldwideweb
Core builds.
...
To build worldwideweb Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      worldwideweb Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/worldwideweb-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/worldwideweb-core/guix.sigs).
