...
Tracing (USDT) support for worldwideweb Core on Linux using. For more information on
USDT support in worldwideweb Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `worldwidewebd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `worldwidewebd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[worldwideweb Core USDT documentation] on how to list available tracepoints in your
`worldwidewebd` binary.

[worldwideweb Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/worldwidewebd
```
...
worldwideweb P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/worldwidewebd
```
...
In a different terminal, starting worldwideweb Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/worldwidewebd -signet -reindex
```
