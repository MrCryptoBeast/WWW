...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing worldwideweb-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='worldwideweb-cli', help='worldwideweb-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='worldwideweb address to which the faucet should send')
...
parser.add_argument('worldwideweb_cli_args', nargs='*', help='Arguments to pass on to worldwideweb-cli (default: -signet)')

args = parser.parse_args()

if args.worldwideweb_cli_args == []:
    args.worldwideweb_cli_args = ['-signet']


def worldwideweb_cli(rpc_command_and_params):
    argv = [args.cmd] + args.worldwideweb_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = worldwideweb_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = worldwideweb_cli(['getnewaddress', 'faucet', 'bech32'])
