...
static void SetupworldwidewebTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " worldwideweb-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  worldwideweb-tx [options] <hex-tx> [commands]  Update hex-encoded worldwideweb transaction\n"
                "or:     worldwideweb-tx [options] -create [commands]   Create hex-encoded worldwideweb transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded worldwideweb transaction
