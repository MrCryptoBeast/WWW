...
#include <qt/worldwidewebaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a worldwideweb address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new worldwidewebAddressEntryValidator(parent));
    widget->setCheckValidator(new worldwidewebAddressCheckValidator(parent));
}
...
bool parseworldwidewebURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no worldwideweb: URI
    if(!uri.isValid() || uri.scheme() != QString("worldwideweb"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!worldwidewebUnits::parse(worldwidewebUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseworldwidewebURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseworldwidewebURI(uriInstance, out);
}

QString formatworldwidewebURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("worldwideweb:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(worldwidewebUnits::format(worldwidewebUnits::BTC, info.amount, false, worldwidewebUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openworldwidewebConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", worldwideweb_CONF_FILENAME));
    ...
    /* Open worldwideweb.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "worldwideweb.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "worldwideweb (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("worldwideweb (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for worldwideweb*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "worldwideweb.desktop";
    return GetAutostartDir() / strprintf("worldwideweb-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a worldwideweb.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=worldwideweb\n";
        else
            optionFile << strprintf("Name=worldwideweb (%s)\n", chain);
