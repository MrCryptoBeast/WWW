This directory contains the source code for the worldwideweb Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### worldwidewebgui.(h/cpp)

- Represents the main window of the worldwideweb UI.
...
- ClientModel is used by the main application `worldwidewebgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a worldwideweb:... link from a browser).
...
* UI elements like worldwidewebAmountField, which inherit from QWidget.
* `worldwidewebstrings.cpp`: automatically generated
* `worldwidewebunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/worldwideweb_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `worldwideweb-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `worldwideweb-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `worldwideweb-qt`
9. You're all set! Start developing, building, and debugging the worldwideweb Core GUI
