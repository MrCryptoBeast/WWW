...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/worldwideweb.conf are parsed in qt/worldwideweb.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  worldwidewebd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see worldwidewebd -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for worldwidewebd but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect worldwidewebd signal handlers
    noui_connect();
