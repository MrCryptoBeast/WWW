...
// For details see https://github.com/worldwideweb/worldwideweb/pull/22348.
...
const char * const worldwideweb_CONF_FILENAME = "worldwideweb.conf";
const char * const worldwideweb_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/worldwideweb/worldwideweb/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //worldwideweb-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", worldwideweb_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "worldwideweb";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\worldwideweb
    // macOS: ~/Library/Application Support/worldwideweb
    // Unix-like: ~/.worldwideweb
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "worldwideweb";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/worldwideweb";
#else
    // Unix-like
    return pathRet / ".worldwideweb";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", worldwideweb_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure worldwideweb Core copyright is not removed by accident
    if (copyright_devs.find("worldwideweb Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The worldwideweb Core developers";
    }
