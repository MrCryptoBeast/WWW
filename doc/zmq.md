...
The worldwideweb Core daemon can be configured to act as a trusted "border
router", implementing the worldwideweb wire protocol and relay, making
...
The ZeroMQ feature in worldwideweb Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building worldwidewebd:
...
For instance:
    ...
    $ worldwidewebd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubhashblock="tcp://[::1]:28333" \
               -zmqpubrawtx=ipc:///tmp/worldwidewebd.tx.raw \
...
These options can also be provided in worldwideweb.conf.
...
From the perspective of worldwidewebd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into worldwidewebd directly. Furthermore, no information is
...
using. worldwidewebd appends an up-counting sequence number to each
