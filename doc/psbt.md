# PSBT Howto for worldwideweb Core

Since worldwideweb Core 0.17, an RPC interface exists for Partially Signed worldwideweb
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/worldwideweb/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for worldwideweb transactions that are not fully signed
...
#### Multisig with multiple worldwideweb Core instances
...
worldwideweb Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `worldwideweb-cli` is used.
