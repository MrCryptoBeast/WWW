worldwideweb Core version 0.12.0 is now available from:

  <https://worldwideweb.org/bin/worldwideweb-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/worldwideweb-Qt (on Mac) or
worldwidewebd/worldwideweb-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of worldwideweb Core or other software:
...
pre-0.12 versions of worldwideweb Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start worldwideweb Core version 0.11 or
earlier.
...
ECDSA signatures inside worldwideweb transactions now use validation using
[libsecp256k1](https://github.com/worldwideweb-core/secp256k1) instead of OpenSSL.
...
(https://github.com/worldwideweb/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of worldwideweb Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of worldwideweb Core was to raise the default minimum
relay fee.

worldwideweb Core 0.12 will have a strict maximum size on the mempool. The
...
worldwideweb Core 0.12 also introduces new default policy limits on the length and
...
worldwideweb Core 0.12 nodes. worldwideweb Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/worldwideweb/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in worldwideweb Core 0.12 does not yet have support for
...
This allows running worldwidewebd without having to do any manual configuration.
...
worldwideweb Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. worldwideweb Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In worldwideweb Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in worldwideweb Core 0.13, and it is at this time undecided whether the
...
worldwideweb Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
worldwideweb Core automatically creates a hidden service to listen on, without
manual configuration. worldwideweb Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if worldwideweb Core is listening, and
...
worldwidewebd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals worldwideweb
Core to use floating fees. By default, worldwideweb Core will use floating
fees.
...
At all times, worldwideweb Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BTC.
Furthermore, worldwideweb Core will never create transactions paying less than
...
`worldwideweb.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/worldwideweb/worldwideweb/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/worldwideweb/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/worldwideweb/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /worldwidewebrpc>
        ...
        # optional bypass worldwidewebd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< worldwidewebrpc:<password>
    </Location>
...
Restarting worldwidewebd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of worldwideweb-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new worldwideweb-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include worldwideweb-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix worldwideweb-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of worldwideweb-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override worldwidewebD and worldwidewebCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [worldwideweb-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add worldwidewebD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libworldwidewebconsensus (Braydon Fuller)
