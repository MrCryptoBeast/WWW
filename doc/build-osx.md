...
This guide describes how to build worldwidewebd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build worldwideweb Core from source.
...
### 4. Clone worldwideweb repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the worldwideweb Core repository to a directory.
...
It is not necessary to build wallet functionality to run `worldwidewebd` or  `worldwideweb-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/worldwideweb/worldwideweb/blob/master/doc/descriptors.md), `sqlite` is required.
...
worldwideweb Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/worldwideweb/worldwideweb/issues/7714).
...
You can deploy a `.dmg` containing the worldwideweb Core application using `make deploy`.
...
## Building worldwideweb Core

### 1. Configuration

There are many ways to configure worldwideweb Core, here are a few common examples:
...
Run the following in your terminal to compile worldwideweb Core:
...
## Running worldwideweb Core

worldwideweb Core should now be available at `./src/worldwidewebd`.
If you compiled support for the GUI, it should be available at `./src/qt/worldwideweb-qt`.

The first time you run `worldwidewebd` or `worldwideweb-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/worldwideweb/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/worldwideweb"

touch "/Users/${USER}/Library/Application Support/worldwideweb/worldwideweb.conf"

chmod 600 "/Users/${USER}/Library/Application Support/worldwideweb/worldwideweb.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/worldwideweb/debug.log
```

## Other commands:

```shell
./src/worldwidewebd -daemon      # Starts the worldwideweb daemon.
./src/worldwideweb-cli --help    # Outputs a list of command-line options.
./src/worldwideweb-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/worldwideweb-qt -server # Starts the worldwideweb-qt server mode, allows worldwideweb-cli control
```
