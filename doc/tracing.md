# User-space, Statically Defined Tracing (USDT) for worldwideweb Core

worldwideweb Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to worldwideweb Core
...
Multiple tools can list the available tracepoints in a `worldwidewebd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in worldwideweb Core, use `info probes` in `gdb`:

```
$ gdb ./src/worldwidewebd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/worldwidewebd
stap net        outbound_message 0x0000000000107c05 /src/worldwidewebd
stap validation block_connected  0x00000000002fb10c /src/worldwidewebd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in worldwideweb Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/worldwidewebd | grep NT_STAPSDT -A 4 -B 2
